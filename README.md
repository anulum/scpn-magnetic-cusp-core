...
owner of device-level truth for the `cusp` configuration of the SCPN Phase
Orchestrator reactor registry (magnetic cusp).
...
serialisation, and a data-only SPO registry pin (`src/scpn_magnetic_cusp_core/`,
...
This repository owns, for the magnetic-cusp device family:

- the device boundary: plant and experiment truth, lifecycle, and
  configuration policy for devices confining plasma with opposed magnetic
  fields whose point and line cusps form a magnetohydrodynamically stable,
  everywhere-convex boundary, with confinement set by the balance between
  high-beta interior, narrow sheath-scale loss apertures at the cusps, and
  cusp-loss transport;
- cusp-geometry semantics (spindle and picket-fence classes, point-cusp and
  line-cusp loss-aperture declarations) as configuration facets;
...
- **Polywell-style electrostatic confinement**: `SCPN-IEC-CORE`. The SCPN
  registry classifies `polywell` under the magnetic-open family for its
  cusp topology, but the portfolio standard intentionally assigns the
  device to the electrostatic owner because its energy and reaction
  workflow is an electrostatic potential well; this repository owns purely
  magnetic cusp confinement only.
- **Magnetic mirrors** (axial mirror-force confinement): `SCPN-MIRROR-CORE`.
...
artefact, and no parameter set describes or validates any real machine. Cusp-geometry and fuel-cycle choices are configuration
