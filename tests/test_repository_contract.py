...
# SCPN Magnetic Cusp Core — repository-level truth tests
...
REQUIRED_PATHS = (
    ...
    "src/scpn_magnetic_cusp_core/__init__.py",
    "src/scpn_magnetic_cusp_core/configuration.py",
    "src/scpn_magnetic_cusp_core/errors.py",
    "src/scpn_magnetic_cusp_core/observability.py",
    "src/scpn_magnetic_cusp_core/plan_envelope.py",
    "tests/data/plan_envelope_fixture.json",
    "src/scpn_magnetic_cusp_core/parameters.py",
...
def test_manifest_declares_exact_configuration_assignment() -> None:
    ...
    assert manifest["project"] == "SCPN-MAGNETIC-CUSP-CORE"
...
def test_package_agrees_with_manifest_truth() -> None:
    ...
    from scpn_magnetic_cusp_core import OWNED_CONFIGURATIONS, RegistryBinding
...
def test_typed_package_marker_exists() -> None:
    ...
    assert (REPO / "src" / "scpn_magnetic_cusp_core" / "py.typed").is_file()
