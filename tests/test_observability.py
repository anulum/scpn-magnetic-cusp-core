...
# SCPN Magnetic Cusp Core — diagnostic and clock semantics tests
...
from scpn_magnetic_cusp_core.errors import DiagnosticPlanError
...
NONCYCLIC_BINDINGS = {
    ...
    "coordinate_frame": "frm_cusp_axis",
    "provenance": "synthetic fixture",
    "quality": "synthetic quality flags",
...
REFERENCE_FRAMES = (
    ReferenceFrame(
        identifier="frm_cusp_axis",
        kind=FrameKind.MACHINE_CYLINDRICAL,
        description="cusp axis cylindrical frame",
    ),
    ReferenceFrame(
        identifier="frm_field_line",
        kind=FrameKind.FIELD_LINE,
        description="field-line following frame through the cusp",
    ),
)
CLOCK_RELATIONS = (
    ClockRelation(
...
SIGNALS_CH_CUSP_LOSS_PROBES = (
    SignalDeclaration(
        identifier="sig_cusp_loss_flux",
        quantity="particle_flux",
        unit="m^-2/s",
        role=SignalRole.CARRIER,
        description="synthetic cusp-loss particle flux",
    ),
    SignalDeclaration(
...
REFERENCE_TRANSFORMATIONS: tuple[FrameTransformation, ...] = (
    FrameTransformation(
        source_identifier="frm_cusp_axis",
...
def channel_noncyclic_one() -> DiagnosticChannelPlan:
    """Build the synthetic cusp-loss probe set channel."""
    return DiagnosticChannelPlan(
        identifier="ch_cusp_loss_probes",
        ...
        signals=SIGNALS_CH_CUSP_LOSS_PROBES,
        synthetic=True,
    )
...
def channel_derived() -> DiagnosticChannelPlan:
    ...
    return DiagnosticChannelPlan(
        ...
        max_signal_frequency_hz=1.0e04,
        timing_uncertainty_s=None,
        acquisition_start_s=0.0,
        acquisition_duration_s=0.5,
        element_count=16,
...
def synthetic_plan() -> DiagnosticPlan:
    ...
    return DiagnosticPlan(
        identifier="cusp_reference_plan",
...
def test_transformation_rejects_self_mapping() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="to itself"):
        _transformation(target_identifier="frm_cusp_axis")
    ...
    with pytest.raises(DiagnosticPlanError, match="duplicate transformation pair"):
        _plan_with(
            frame_transformations=(
                ...
                _transformation(
                    source_identifier="frm_field_line",
                    target_identifier="frm_cusp_axis",
                ),
            )
        )
...
def test_report_flags_band_outside_typical_range() -> None:
    ...
    findings = plan.consistency_report()
    assert len(findings) == 1
    assert "Berkowitz" in findings[0].message
...
def test_plan_rejects_unknown_frame_reference() -> None:
    ...
    plan = synthetic_plan()
    kept = tuple(frame for frame in plan.frames if frame.identifier != "frm_cusp_axis")
