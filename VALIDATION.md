...
SCPN Magnetic Cusp Core — VALIDATION
-->
...
python3 agentic-shared/scripts/repository_tier0_scaffold_audit.py \
  03_CODE/SCPN-MAGNETIC-CUSP-CORE --json
```
...
- Validated frozen parameter objects (`CuspGeometry`, `CoilDrive`,
  `DeviceConfiguration`) rejecting non-finite values, non-positive
  extents, and the hard class invariants: opposed neighbouring currents
  (the defining central field null; Berkowitz et al., 1958), exactly
  two coils for the spindle class, and an even count of at least four
  for the picket fence — every rejection branch is tested.
- The standard on-axis loop-field formula
  `B(z) = mu0 I R^2 / (2 (R^2 + z^2)^{3/2})` and the spindle-pair
  coil-plane helper built on it.
- An advisory consistency finding with a documented model bound,
  reported and never clamped: a spindle separation-to-radius ratio
  outside `[0.5, 4]`.
...
- The estimates are advisory regime checks, not confinement or
  stability results; no benchmark, dataset, solver, controller, or
...
- A reference plan mirroring canonical practice with synthetic
  declarations: cusp-loss probes, diamagnetic loop, drive reference, flute probe array, synthetic oscillator, each bound to its clock domain.
- Documented advisory band checks with their sources stated in the
  code: cusp interchange/flute activity in the 0.1–100 kHz scale and ICRF-to-ECRH drive frequencies (Berkowitz et al. 1958); findings are reported, never clamped.
- Canonical serialisation (sorted keys, NaN/infinity rejected on both
  emit and parse), SHA-256 digest identity, and a strict round-trip
  parser that refuses unknown fields.
...
(`src/scpn_magnetic_cusp_core/plan_envelope.py`,
