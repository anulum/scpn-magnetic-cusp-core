...
# SCPN Magnetic Cusp Core — diagnostic and clock semantics
...
identities for the magnetic-cusp configuration this repository owns,
...
from scpn_magnetic_cusp_core.configuration import ConsistencyFinding
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-MAGNETIC-CUSP-CORE"
                )
...
class DiagnosticChannelPlan:
    ...
    timing_uncertainty_s
        ...
        candidate is applicable to SCPN-MAGNETIC-CUSP-CORE.
    acquisition_start_s
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-MAGNETIC-CUSP-CORE"
            )
        if not self.reason:
...
class DiagnosticPlan:
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            Findings from documented cusp diagnostic practice;
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "Hz is outside the cusp low-frequency "
                            "instability scale 0.1-100 kHz "
                            "(Berkowitz et al. 1958)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "1 MHz-200 GHz used on cusp experiments "
                            "(Post 1987)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the cusp confinement scale of up to ~1 "
                            "s (Berkowitz et al. 1958)"
                        ),
                    )
                )
