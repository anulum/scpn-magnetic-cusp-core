...
SCPN Magnetic Cusp Core — CONTROL adapter specification
-->

# CONTROL adapter specification

**Adapter identifier:** `scpn-magnetic-cusp-core.control-adapter`  
...
magnetic-cusp adapter would supply device truth to the SCPN control plane.
...
- channel identifier and physical quantity with SI unit — including
  point-cusp and line-cusp loss fluxes and boundary-sharpness (beta
  regime) indicators with their exact definitions declared per channel;
- coordinate convention (cusp axis, midplane ring, loss-aperture label)
  and spatial layout (scalar, aperture array, or profile);
...
- lifecycle phase (`field_ramp`, `filling`, `confined`, `termination`,
  `terminated`) with monotonic transition semantics and device-truth event
  records for beta-collapse (loss of the sharp-boundary regime);
- configuration identity: the `cusp` registry configuration, the cusp
  geometry class (spindle, picket-fence), and the configuration policy
  revision that produced the run;
...
For each actuator class (opposed-coil current programming, filling systems
(plasma guns, gas valves), auxiliary heating):
...
- machine-readable operational envelope (coil currents and opposed-field
  forces, density and beta bounds, loss-flux load limits with
  applicability domains);
- declared device-level hazard semantics for beta collapse and coil-force
  excursions;
...
above through CONTROL's real admission path: nominal confined runs,
beta-collapse sequences, boundary-of-envelope cases, invalid-payload
