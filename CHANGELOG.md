...
SCPN Magnetic Cusp Core — CHANGELOG
-->
...
- Generated repository header artwork: `docs/assets/generate_header.py`
  ...
  domain surface (the spindle cusp with its computed central null used
  by the README, the two coil classes, and the minimum-B map).
...
- Diagnostic and clock semantics model
  (`src/scpn_magnetic_cusp_core/observability.py`), the second implemented
...
- Device configuration model (`src/scpn_magnetic_cusp_core/`), the first implemented
